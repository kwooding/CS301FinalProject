...
def predict_values(n_clicks, feature_input, selected_features, data, target_column):
    if n_clicks > 0:
        ...
        # Adjust prediction for working professionals to 0 or 1 based on threshold
        adjusted_prediction = 1 if prediction > 0.5 else 0
        return f"Predicted {target_column}: {adjusted_prediction}"
